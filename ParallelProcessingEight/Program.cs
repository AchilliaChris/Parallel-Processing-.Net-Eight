...
namespace ParallelProcessingEight
{
    internal class Program
    {
        ...
        static async Task Run(string[] args, IBespokeDictionary dictionary)
        {
            ...
            Console.WriteLine("Starting the process at {0}", StartTime.ToString());
            ...
            Console.WriteLine();
            count = await MeasureAsync("Vat added using dictionary.ParallelForVat().", () => Task.Run(() => concrete.ParallelForVat(rate)));
            ...
            Console.WriteLine();
            count = await MeasureAsync("Vat added using dictionary.ParallelForEachVat().", () => Task.Run(() => concrete.ParallelForEachVat(rate)));
            ...
            Console.WriteLine();
            count = await MeasureAsync("Vat added using dictionary.ParallelForOptionsVat().", () => Task.Run(() => concrete.ParallelForOptionsVat(parallelOptions, rate)));
            ...
            if (concrete.widgets.TryGetValue(key, out parpostOptionsAsyncVat))
                ...
                Console.WriteLine("Key not found after ParralelForEachAsyncOptionsVat: " + key);
            stopWatch.Stop();
        ...
        private static IHostApplicationBuilder CreateHost(string[] args)
        {
            HostApplicationBuilder builder = Host.CreateApplicationBuilder(args);
            builder.Services.AddSingleton<IBespokeDictionary, BespokeDictionary>();
